import { ADD_MEMBER, DELTE_MEMBER, MODIFY_MEMBER, GET_MEMBER, RECEIVE_MEMBER, fetchMembersFromServer, serverUrl } from '../actions'
...
const deleteMemberFromServer = (members) => {
  ...
  if (member.length > 0) {
    fetch(serverUrl + `/members/delete`, {
      method: "POST",
...
const addMemberToServer = async(members, dispatch, tenantid) => {
  ...
  if (member.length > 0) {
      await fetch(serverUrl + `/members/add`, {
      method: "POST",
      body: JSON.stringify(member),
      headers: {
        'Accept': 'application/json',
...
const updateMemberToServer = async(members, id, dispatch, columnName) => {
  ...
  if (member.length > 0) {
      await fetch(serverUrl + `/members/update`, {
      method: "POST",
